...
output_main_folder = main_path+'classification_DR11_4MOST_extragalactic_2023/classifications_and_features/'
...
#    file = input_files_names[i]
    ...
    if len(y_pred_prob_var_transient)>0:
    ...
    X_nonvar.loc[:,'pred_init_class']=y_pred_init[np.where(y_pred_init!='variable')]
    X_nonvar.loc[:,'pred_init_class_prob']=np.max(y_pred_prob_init[np.where(y_pred_init!='variable')],axis=1)
